...
// const rows = [
//   createData("KitKat", "kitkat@gmail.com", "Sweet"),
// ];
...
export default function EnhancedTable() {
  ...
  const getData = async () => {
    ...
    ); // 2. change it to live service URL
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = commentsData.map((n) => n.name);
      ...
      <Paper className={classes.paper}>
        ...
        <TableContainer className={classes.tablecontainer}>
          ...
          >
            ...
            <TableBody>
              {stableSort(commentsData, getComparator(order, orderBy))
              ...
              {/* Leave empty space if page size < page limit */}
              {/* {emptyRows > 0 && (
                <StyledTableRow
